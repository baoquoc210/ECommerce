        ...
        .Buttons(
          {
            createOrder: async () => {
              ...
              const resp = await fetch(
                "https://v6.exchangerate-api.com/v6/8f85eea95dae9336b9ea3ce9/latest/AUD"
              );
              ...
              if (respData.result === "success") {
                const usdToAudRate = respData.conversion_rates.USD;
                convertedAmount = (totalAmount * usdToAudRate).toFixed(2);
              }
                  ...
                  context?.cartData?.length !== 0 && context?.cartData?.map((item, index) => {
                    return (
                      <div className="flex items-center justify-between py-2" key={index}>
                        ...
                        <span className="text-[14px] font-[500]">{(item?.quantity * item?.price)?.toLocaleString('en-US', { style: 'currency', currency: 'AUD' })}</span>
                      </div>
                    )
                  })
                }
